    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="mb-6">
        <div className="relative max-w-md">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari PB..." className="w-full rounded-xl border border-gray-200 bg-white py-2.5 pl-10 pr-4 text-sm text-gray-900 placeholder:text-gray-400 shadow-sm focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/10" />
        </div>
      </div>
      ...
      <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col items-end gap-2">
            ...
            {bgSaved && <span className="text-xs font-semibold text-[var(--color-primary)]">✓ Tersimpan</span>}
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-gray-200 bg-white px-6 py-16 text-center shadow-sm">
          ...
          {pbs.length === 0 && <button onClick={openAdd} className="mt-3 text-sm font-medium text-[var(--color-primary)] hover:underline">Buat PB pertama</button>}
        </div>
      ) : (
            ...
            <div key={pb.id} className="group rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition-all hover:shadow-md">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className={`flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-xl text-xl shadow-sm ${pb.logoUrl ? "" : "bg-[var(--color-primary)]"}`}>
                    {pb.logoUrl ? <img src={pb.logoUrl} alt={pb.name} className="h-full w-full object-cover" /> : "🏸"}
                  </div>
                  <div>
                    <Link href={`/admin/pbs/${pb.id}`} className="font-semibold text-gray-900 hover:text-[var(--color-primary)] transition-colors">{pb.name}</Link>
              ...
              <Link href={`/admin/pbs/${pb.id}`} className="mt-4 flex items-center justify-between rounded-xl bg-gray-50 px-4 py-2.5 text-xs font-medium text-gray-600 transition-colors hover:bg-[var(--color-primary-light)] hover:text-[var(--color-primary)]">
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl sm:p-8 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" disabled={saving} className="rounded-xl bg-[var(--color-primary)] px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-[var(--color-primary-hover)] hover:shadow-md disabled:opacity-50 disabled:cursor-not-allowed">{saving ? "Menyimpan..." : "Tambah PB"}</button>
              </div>
            </form>
          </div>
        </div>
