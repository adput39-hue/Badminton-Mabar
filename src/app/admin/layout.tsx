...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!checked || !user) return <div className="flex min-h-screen items-center justify-center bg-[var(--color-bg)]"><div className="h-8 w-8 animate-spin rounded-full border-4 border-[var(--color-primary)] border-t-transparent" /></div>;
    ...
    <div className="flex min-h-screen bg-[var(--color-bg)]">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex flex-col border-r border-gray-200 bg-white transition-all duration-300 lg:static lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} ${collapsed ? "w-16" : "w-60"}`}>
        <div className="flex items-center gap-3 border-b border-gray-100 px-5 py-4 min-h-[68px]">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--color-primary)] text-xl shrink-0">🏸</div>
          {!collapsed && (
            <div className="overflow-hidden">
              <p className="text-sm font-bold text-gray-900 truncate">Admin Panel</p>
              <p className="text-xs text-[var(--color-primary)] font-medium">Master PB</p>
            </div>
          )}
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)}
                className={`flex items-center gap-3 rounded-xl px-4 py-2.5 text-sm font-medium transition-all ${collapsed ? "justify-center px-0" : ""} ${
                  isActive(item.href) ? "bg-[var(--color-primary)] text-white shadow-sm" : "text-gray-600 hover:bg-gray-100 hover:text-gray-900"
                }`}>
        ...
        <div className="border-t border-gray-100 p-3">
          <div className="flex items-center gap-3 rounded-xl px-3 py-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[var(--color-primary)] text-xs font-bold text-white shrink-0">
      ...
      <div className="flex flex-1 flex-col min-w-0">
        <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-gray-200 bg-white/90 px-4 py-3 backdrop-blur sm:px-6">
          ...
          <div className="flex items-center gap-2 rounded-xl bg-[var(--color-primary-light)] px-3 py-1.5">
            <Shield className="h-4 w-4 text-[var(--color-primary)]" />
            <span className="text-xs font-semibold text-[var(--color-primary)]">Super Admin</span>
          </div>
